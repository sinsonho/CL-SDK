...
*구축 가이드 · RHEL 10 (aarch64) · root 실행*
...
> **이 문서 버전**
>
> 대상 운영체제: Red Hat Enterprise Linux 10 10.2
>
> 아키텍처(CPU 종류): aarch64 (ARM 64비트)
>
...
→ 이 RHEL 10 aarch64 문서는 위 표에서 **‘arm64 설치 = 가능’** 인 컴퓨터에서 진행하세요.
...
이 문서는 **Red Hat Enterprise Linux 10 10.2 (aarch64)** 용입니다. 아래에서 해당 ISO 파일 1개를 내려받으세요. 가장 무난한 것은 ‘DVD’ 이미지입니다.

| 항목 | 내용 |
| --- | --- |
| 배포판/버전 | Red Hat Enterprise Linux 10 10.2 |
| 아키텍처 | aarch64 (ARM 64비트) |
| 내려받을 파일 | rhel-10.2-aarch64-dvd.iso (aarch64 · DVD) |
| 공식 다운로드 | [developers.redhat.com/products/rhel/download](https://developers.redhat.com/products/rhel/download) |

> **다운로드 참고**
>
> RHEL은 무료 Red Hat 개발자 계정(Developer Subscription for Individuals)으로 가입하면 ISO를 내려받을 수 있습니다. 페이지에서 ‘Red Hat Enterprise Linux 10.2 · aarch64 · DVD’를 선택하세요.
...
- 두 서버 모두 RHEL 10 (aarch64), root 계정으로 작업합니다.
...
cl-sdk는 ipykernel 을 포함하므로, 같은 venv에 Kernel Gateway만 추가하면 ‘Python 3’ 커널에서 곧바로 import cl 을 사용할 수 있습니다. pip가 현재 아키텍처(aarch64)에 맞는 aarch64(arm64)용 wheel을 자동으로 설치합니다.
...
RHEL 10은 기본 Python이 3.12입니다. 보통 이미 설치되어 있습니다.
...
Notebook 7.x가 설치됩니다. aarch64용 wheel이 제공되어 별도 빌드 도구 없이 설치됩니다.
