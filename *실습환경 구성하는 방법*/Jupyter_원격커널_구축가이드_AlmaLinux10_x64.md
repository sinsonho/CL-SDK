...
*구축 가이드 · AlmaLinux 10 (x86_64) · root 실행*
...
→ 이 AlmaLinux 10 x86_64 문서는 위 표에서 **‘x64 설치 = 가능’** 인 컴퓨터에서 진행하세요.
...
이 문서는 **AlmaLinux OS 10 10.2 (x86_64)** 용입니다. 아래에서 해당 ISO 파일 1개를 내려받으세요. 가장 무난한 것은 ‘DVD’ 이미지입니다.

| 항목 | 내용 |
| --- | --- |
| 배포판/버전 | AlmaLinux OS 10 10.2 |
| 아키텍처 | x86_64 (Intel/AMD 64비트) |
| 내려받을 파일 | AlmaLinux-10-latest-x86_64-dvd.iso (x86_64 · DVD) |
| 공식 다운로드 | [almalinux.org/get-almalinux](https://almalinux.org/get-almalinux/) |

> **다운로드 참고**
>
> 계정 없이 바로 내려받을 수 있습니다. AlmaLinux OS 10 · x86_64 · DVD를 선택하세요. (직접 링크: repo.almalinux.org/almalinux/10/isos/x86_64/) ※ 아주 오래된 CPU라면 같은 페이지의 x86_64_v2 버전을 받으면 됩니다(AlmaLinux는 v2도 별도 제공).
...
>
> AlmaLinux 10 10 계열은 x86-64-v3 명령어셋을 요구합니다. 2015년 이후 대부분의 PC는 문제없지만, VMware에서 CPU 호환성을 낮게(예: ‘이전 버전 호환’) 설정하면 부팅이 안 될 수 있으니 기본값으로 두세요.
...
- 두 서버 모두 AlmaLinux 10 (x86_64), root 계정으로 작업합니다.
