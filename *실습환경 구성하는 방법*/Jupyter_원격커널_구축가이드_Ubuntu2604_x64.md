...
*구축 가이드 · Ubuntu 26.04 LTS (x86_64) · root 실행*
...
→ 이 Ubuntu 26.04 LTS x86_64 문서는 위 표에서 **‘x64 설치 = 가능’** 인 컴퓨터에서 진행하세요.
...
이 문서는 **Ubuntu Server (Resolute Raccoon) 26.04 LTS (x86_64)** 용입니다. 아래에서 해당 ISO 파일 1개를 내려받으세요. 가장 무난한 것은 ‘DVD’ 이미지입니다.

| 항목 | 내용 |
| --- | --- |
| 배포판/버전 | Ubuntu Server (Resolute Raccoon) 26.04 LTS |
| 아키텍처 | x86_64 (Intel/AMD 64비트) |
| 내려받을 파일 | ubuntu-26.04-live-server-amd64.iso (amd64 · Server) |
| 공식 다운로드 | [ubuntu.com/download/server](https://ubuntu.com/download/server) |

> **다운로드 참고**
>
> Ubuntu는 계정 없이 바로 내려받을 수 있습니다. ‘Ubuntu Server 26.04 LTS’의 amd64(x86_64) ISO를 받으세요. 데스크톱(GUI) 버전은 필요 없습니다.
...
>
> 설치 유형은 ‘Ubuntu Server’ 기본값으로 진행하면 됩니다(데스크톱 GUI는 필요 없습니다).
...
- 두 서버 모두 Ubuntu 26.04 LTS (x86_64), root 계정으로 작업합니다.
