...
*구축 가이드 · Rocky Linux 10 (aarch64) · root 실행*
...
→ 이 Rocky Linux 10 aarch64 문서는 위 표에서 **‘arm64 설치 = 가능’** 인 컴퓨터에서 진행하세요.
...
이 문서는 **Rocky Linux 10 10.2 (aarch64)** 용입니다. 아래에서 해당 ISO 파일 1개를 내려받으세요. 가장 무난한 것은 ‘DVD’ 이미지입니다.

| 항목 | 내용 |
| --- | --- |
| 배포판/버전 | Rocky Linux 10 10.2 |
| 아키텍처 | aarch64 (ARM 64비트) |
| 내려받을 파일 | Rocky-10-latest-aarch64-dvd.iso (aarch64 · DVD) |
| 공식 다운로드 | [rockylinux.org/download](https://rockylinux.org/download) |

> **다운로드 참고**
>
> 계정 없이 바로 내려받을 수 있습니다. 다운로드 페이지에서 Rocky Linux 10 · aarch64 · DVD를 선택하세요. (직접 링크: download.rockylinux.org/pub/rocky/10/isos/aarch64/)
...
- 두 서버 모두 Rocky Linux 10 (aarch64), root 계정으로 작업합니다.
